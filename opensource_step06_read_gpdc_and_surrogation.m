...
included_ids = [cellfun(@(x) str2double(['1' x]), uk_list), ...
                cellfun(@(x) str2double(['2' x]), sg_list)];

fprintf('Analysis sample:\n');
fprintf('  Behavioral sample reported in manuscript: 47 infants\n');
fprintf('  EEG analysis sample: 42 infants\n\n');
...
for obs_idx = 1:size(behavioral_data, 1)
    ...
    % Retain EEG-valid participants for connectivity analyses
    if ~ismember(participant_id, included_ids)
        continue;
    end
...
% This aggregation step saves real GPDC observations. Surrogate outputs are
% generated in the surrogate-testing step and can be merged downstream.
if exist('data_surr', 'var')
